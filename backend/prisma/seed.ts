...
async function main() {
  ...
  // 12. GRADE APPEALS (Đơn phúc khảo dữ liệu thật)
  console.log(' └─ Grade Appeals (Đơn phúc khảo dữ liệu thật)...');
  const allSts = await prisma.student.findMany({ include: { class: true }, take: 6 });
  const allScheds = await prisma.examSchedule.findMany({ include: { subject: true }, take: 6 });

  const sampleAppealsData = [
    {
    ...
    {
      reason: 'Kính xin Thầy/Cô xem xét lại bài tự luận môn Phân tích Thiết kế Hệ thống, sơ đồ tuần tự của em có vẽ đúng các thông điệp đồng bộ.',
      status: 'PENDING',
      originalScore: 6.0,
      revisedScore: null,
      reviewerNote: null,
  ...
  const firstPaper = await prisma.examPaper.findFirst();

  for (let i = 0; i < sampleAppealsData.length; i++) {
    const st = allSts[i % allSts.length];
    const sched = allScheds[i % allScheds.length];
    const item = sampleAppealsData[i];

    if (st && sched) {
      let cfg = await prisma.onlineExamConfig.findFirst({
        where: { examScheduleId: sched.id },
      });
      if (!cfg && firstPaper) {
        cfg = await prisma.onlineExamConfig.create({
          data: {
            examScheduleId: sched.id,
            examPaperId: firstPaper.id,
            accessCode: `EXAM-${sched.id}`,
      ...
      if (cfg) {
        let att = await prisma.examAttempt.findFirst({
          where: { onlineExamConfigId: cfg.id, studentId: st.id },
        });
        if (!att) {
          att = await prisma.examAttempt.create({
            data: {
              onlineExamConfigId: cfg.id,
              studentId: st.id,
              attemptNumber: 1,
              attemptToken: `TOKEN-${st.id}-${sched.id}-${i}`,
              status: 'SUBMITTED',
              totalScore: item.originalScore,
              maxScore: 10,
        ...
        const existingAppeal = await prisma.gradeAppeal.findFirst({
          where: { attemptId: att.id, studentId: st.id },
        });
