...
import {
  CalendarClock, CheckCircle2, Cloud, DatabaseBackup, Edit3, ExternalLink,
...
export default function SystemSettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 pb-12 sm:p-6">
      ...
      <form onSubmit={saveSchedule} className="grid gap-5 xl:grid-cols-[1fr_0.8fr]">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="grid gap-4 p-5 lg:grid-cols-2">
          {settings.storageTargets.map((target) => {
            ...
            return <article key={target.id} className={`rounded-2xl border p-4 transition ${target.role === 'PRIMARY' ? 'border-blue-300 bg-blue-50/40 dark:border-blue-800 dark:bg-blue-950/20' : 'border-slate-200 dark:border-slate-700'}`}>
              ...
              <div className="mt-4 flex flex-wrap justify-end gap-2">
                {target.provider === 'GOOGLE_DRIVE' && <Button variant="soft" size="sm" leftIcon={<ExternalLink className="h-4 w-4" />} onClick={() => void connectGoogle(target)} isLoading={busyId === target.id}>{target.config.googleConnected ? 'Kết nối lại Google' : 'Kết nối Google'}</Button>}
                <Button variant="secondary" size="sm" onClick={() => void testTarget(target)} isLoading={busyId === target.id}>Kiểm tra</Button>
                <Button variant="secondary" size="sm" leftIcon={<Edit3 className="h-4 w-4" />} onClick={() => { setEditing(target); setModalOpen(true); }}>Sửa</Button>
                <Button variant="danger-outline" size="sm" leftIcon={<Trash2 className="h-4 w-4" />} onClick={() => setDeleting(target)} disabled={target.role === 'PRIMARY'}>Xóa</Button>
              </div>
            </article>;
          })}
