      ...
      <main className="w-full px-6 py-6 space-y-5 min-h-screen text-slate-900 dark:text-slate-100">
        ...
        {activeMainTab === 'summary' ? (
          ...
          <>
            ...
            {/* Modal popup */}
            {showSchedulePicker && typeof document !== 'undefined' && createPortal(
              <div role="dialog" aria-modal="true" aria-label="Chọn Lịch thi để xem Báo cáo" className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                <div className="relative z-[101] w-full max-w-3xl rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl overflow-hidden flex flex-col max-h-[calc(100dvh-1.5rem)] animate-modal-dialog will-change-transform sm:max-h-[calc(100dvh-2rem)]">
                  ...
                  {/* Filter Toolbar */}
                  <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-800/40 flex flex-wrap gap-2.5 items-center">
                    <div className="relative flex-1 min-w-[200px]">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-type-body font-medium text-slate-800 dark:text-slate-100 focus:outline-none placeholder-slate-400"
                      />
                    </div>

                    <FilterSelect
                      value={modalModeFilter}
                      onChange={(e) => setModalModeFilter(e.target.value as any)}
                    ...
                    >
                      <option value="ALL">Dạng thi: Tất cả</option>
                    ...
                    {(modalSearch || modalModeFilter !== 'ALL' || modalSubjectFilter !== 'ALL' || modalStatusFilter !== 'ALL') && (
                      <Button
                        variant="secondary"
                        size="sm"
                        onClick={() => {
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-2 divide-y divide-slate-100 dark:divide-slate-800">
                    {modalFilteredSchedules.length === 0 ? (
                      <div className="py-12 text-center text-slate-400 font-semibold">
                        Không tìm thấy lịch thi nào phù hợp với bộ lọc.
                      </div>
                    ) : (
                      modalFilteredSchedules.map((s: any) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between text-type-body-sm text-slate-500 dark:text-slate-400">
