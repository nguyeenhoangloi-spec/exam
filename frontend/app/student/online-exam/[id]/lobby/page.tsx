...
export default function StudentExamLobbyPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/60 text-slate-900 py-10 px-4 sm:px-6 flex flex-col justify-center items-center">
      ...
      {/* ── Seamless Single White Card Architecture ── */}
      <div className="w-full max-w-2xl bg-white rounded-2xl border border-slate-200/90 shadow-sm p-6 sm:p-8 space-y-6">
        ...
        <div className="space-y-3 pb-5 border-b border-slate-100">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            >
              <ArrowLeft className="w-3.5 h-3.5" /> Quay lại Lịch thi
            </button>

            <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-600 bg-blue-50/80 px-3 py-1 rounded-full border border-blue-100/60">
              <Sparkles className="w-3 h-3 text-blue-500" />
              {examTypeBadgeText}
            </span>
          </div>

          <div className="flex items-start justify-between gap-4 flex-wrap sm:flex-nowrap pt-1">
            <div className="space-y-1">
              <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight leading-tight">
                {schedule?.subject?.subjectName || examInfo?.subjectName || 'Bài Thi Trực Tuyến'}
              </h1>
              <p className="text-xs sm:text-sm font-medium text-slate-500">
                Mã môn: <strong className="text-slate-800 font-semibold">{schedule?.subject?.subjectCode || examInfo?.subjectCode || '---'}</strong>
                <span className="mx-2">•</span>
                Kỳ thi: <strong className="text-slate-800 font-semibold">{schedule?.examPeriod?.name || examInfo?.examPeriodName || '---'}</strong>
              </p>
            </div>

            <div className="inline-flex items-center gap-2 bg-slate-100/80 px-3.5 py-2 rounded-xl text-slate-700 shrink-0 border border-slate-200/60 self-start">
              <Clock className="w-4 h-4 text-slate-500" />
              <div className="text-xs font-semibold">
                <span className="text-slate-400 font-medium block text-[10px] uppercase tracking-wider">Thời gian</span>
                <span className="text-sm font-bold text-slate-900">{durationMinutes} phút</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 rounded-xl bg-slate-50/70 border border-slate-200/60">
                <div className="space-y-0.5">
                  <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider block">Thí sinh</span>
                  <span className="text-xs font-bold text-slate-900 block truncate">{fullName}</span>
                </div>

                <div className="space-y-0.5">
                  <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider block">Mã sinh viên</span>
                  <span className="text-xs font-bold text-slate-900 block tabular-nums">{studentCode}</span>
                </div>

                <div className="space-y-0.5">
                  <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider block">Lớp & Số ghế</span>
                  <span className="text-xs font-bold text-slate-900 block truncate">{studentClass} • Ghế {seatNumber}</span>
                </div>

                <div className="space-y-0.5">
                  <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider block">Phòng & Ca thi</span>
                  <span className="text-xs font-bold text-blue-600 block truncate">{roomName} ({building})</span>
                  <span className="text-[11px] font-medium text-slate-500 block">{timeSlotStr}</span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-100 space-y-4">
              {config?.requireRulesAcceptance !== false && (
                <label className="flex items-start gap-3 text-xs font-medium text-slate-700 cursor-pointer select-none">
                  <input
                    type="checkbox"
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-900">Nhập mật khẩu phòng thi</h3>
                  <p className="text-xs text-slate-500 font-medium">Xác thực quyền truy cập trước khi vào thi</p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setShowPasswordModal(false)}
                className="rounded-lg p-1 text-slate-400 hover:text-slate-600 transition cursor-pointer"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
