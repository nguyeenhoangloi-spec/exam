...
import { Clock, Shield, Flag, CheckCircle, AlertTriangle, Wifi, WifiOff, Send } from 'lucide-react';
...
export default function StudentExamTakePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col select-none">
      <header className="bg-slate-900 border-b border-slate-800 px-6 py-3 sticky top-0 z-30 flex items-center justify-between shadow-lg">
        <div className="flex items-center space-x-4">
          <span className="font-bold text-white text-lg tracking-tight">{attemptData.paperTitle}</span>
          <span className="hidden md:inline-flex items-center px-2.5 py-1 bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-medium rounded-full">
        ...
        <div className="flex items-center space-x-6">
          ...
          <div
            className={`flex items-center space-x-2 px-4 py-1.5 rounded-xl border font-mono text-base font-bold ${
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
              <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="px-2.5 py-0.5 bg-slate-800 text-slate-300 text-xs rounded-full border border-slate-700">
                    {currentQ.score} điểm
                  </span>
                </div>
              ...
              <div className="text-slate-100 text-base md:text-lg leading-relaxed font-medium mb-6">
                {currentQ.content}
              </div>
        ...
        <aside className="w-full md:w-80 bg-slate-900 border-l border-slate-800 p-6 flex flex-col shrink-0">
          ...
          <div className="flex items-center justify-between text-xs text-slate-400 mb-4 pb-3 border-b border-slate-800">
            <span className="flex items-center">
              <span className="w-3 h-3 rounded bg-indigo-600 mr-1.5"></span> Đã làm ({answeredCount})
            </span>
            <span className="flex items-center">
          ...
          <div className="grid grid-cols-5 gap-2 overflow-y-auto flex-1 max-h-[60vh] md:max-h-none pr-1">
            {questions.map((q: any, idx: number) => {
              ...
              let btnStyle = 'bg-slate-950 border-slate-800 text-slate-400 hover:bg-slate-800';
              if (isFlagged) {
              ...
              if (isCurrent) {
                btnStyle += ' ring-2 ring-indigo-400 ring-offset-2 ring-offset-slate-950';
              }
      ...
      {showSubmitModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-white mb-2">Xác Nhận Nộp Bài Thi</h3>
            <p className="text-slate-400 text-sm mb-6">Bạn có chắc chắn muốn kết thúc và nộp bài thi ngay bây giờ?</p>

            <div className="bg-slate-950 p-4 rounded-xl space-y-2 text-sm mb-6 border border-slate-800">
