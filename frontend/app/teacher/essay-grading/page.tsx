...
export default function EssayGradingPage() {
  ...
  return <div className="min-h-screen bg-slate-50 p-6 md:p-8 text-slate-900">
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {selected?.attemptAnswers?.[0] && <button onClick={() => aiSuggest(selected.attemptAnswers[0])} disabled={Boolean(aiLoading)} className="rounded-lg border border-violet-300 bg-violet-50 px-4 py-2 text-sm font-bold text-violet-700">{aiLoading ? 'AI đang chấm...' : '✨ AI đề xuất cho câu đang chọn'}</button>}
