...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside
      className={`sidebar-aside fixed top-0 left-0 z-40 flex flex-col h-screen bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 border-r border-slate-200/70 dark:border-slate-700/70 transition-all duration-300 ease-in-out select-none ${
    ...
    >
      ...
      <div ref={footerRef} className="relative border-t border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-2 shrink-0">
        ...
        {showUserMenu && (
          <div
            className={`absolute bottom-full mb-2 w-56 rounded-2xl border border-slate-200/90 dark:border-slate-700 bg-white dark:bg-slate-900 p-1.5 shadow-xl shadow-slate-200/50 dark:shadow-slate-950/60 z-50 animate-in fade-in zoom-in-95 duration-150 backdrop-blur-xl ${collapsed ? 'left-14' : 'left-2'
              }`}
          >
            {/* Header profile info */}
