...
export const Toast: React.FC<ToastProps> = ({ message, type, onClose }) => {
  ...
  return (
    ...
    >
      {type === 'success' ? <CheckCircle2 className="w-5 h-5 shrink-0 mt-0.5 sm:mt-0" /> : <AlertCircle className="w-5 h-5 shrink-0 mt-0.5 sm:mt-0" />}
      <span className="text-xs sm:text-sm font-semibold leading-relaxed flex-1">{message}</span>
      <button
        type="button"
