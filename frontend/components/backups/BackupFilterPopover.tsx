...
export function BackupFilterPopover({
  ...
  const categories = [
    ...
    { id: 'types' as FilterCategory, label: 'Loại sao lưu', shortcut: '2', icon: Database, badge: filterType ? '1' : null },
    { id: 'statuses' as FilterCategory, label: 'Trạng thái', shortcut: '3', icon: CheckCircle2, badge: filterStatus ? '1' : null },
  ];
  ...
  return (
    <div className="relative inline-block">
      ...
      <button
        ...
        title="Mở bảng điều khiển bộ lọc snapshot sao lưu"
      >
      ...
      {isOpen && mounted &&
        createPortal(
          <div
            ...
            style={popoverStyle}
            role="dialog"
            aria-label="Bảng bộ lọc snapshot sao lưu"
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-900 dark:text-slate-100 leading-none">
                    Bộ lọc snapshot sao lưu
                  </h4>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                    Lọc theo loại snapshot & trạng thái hoàn tất
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-12 min-h-0 flex-1 overflow-hidden">
              ...
              <div className="col-span-8 p-3 overflow-y-auto custom-scrollbar">
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      <div className="min-w-0 pr-2">
                        <div className={`text-xs font-bold ${filterStatus === 'SUCCEEDED' ? 'text-blue-700 dark:text-blue-300' : 'text-slate-800 dark:text-slate-200'}`}>
                          Sao lưu thành công
                        </div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 truncate">
                          Các bản snapshot sẵn sàng khôi phục
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        {filterStatus === 'SUCCEEDED' && <Check className="h-4 w-4 text-blue-600 dark:text-blue-400" />}
                      </div>
                    </button>
                    ...
                    >
                      <div className="min-w-0 pr-2">
                        <div className={`text-xs font-bold ${filterType === 'DATABASE' ? 'text-blue-700 dark:text-blue-300' : 'text-slate-800 dark:text-slate-200'}`}>
                          Chỉ Cơ sở dữ liệu (DATABASE)
                        </div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 truncate">
                          Bản snapshot SQL cơ sở dữ liệu PostgreSQL
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className={`rounded-md px-1.5 py-0.5 text-[11px] font-bold ${
                        ...
                        {filterType === 'DATABASE' && <Check className="h-4 w-4 text-blue-600 dark:text-blue-400" />}
                      </div>
                    </button>
                    ...
                    >
                      <div className="min-w-0 pr-2">
                        <div className={`text-xs font-bold ${filterType === 'FULL' ? 'text-blue-700 dark:text-blue-300' : 'text-slate-800 dark:text-slate-200'}`}>
                          Toàn bộ hệ thống (FULL)
                        </div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 truncate">
                          Bao gồm cả Database và toàn bộ File Uploads
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className={`rounded-md px-1.5 py-0.5 text-[11px] font-bold ${
                        ...
                        {filterType === 'FULL' && <Check className="h-4 w-4 text-blue-600 dark:text-blue-400" />}
                      </div>
                    </button>
                  </div>
                )}
                ...
                {activeCategory === 'types' && (
                  <div className="space-y-1.5">
                    {[
                      { key: '', label: 'Tất cả loại sao lưu', count: counts.total },
                      { key: 'FULL', label: 'Sao lưu Toàn bộ (FULL)', count: counts.full },
                      { key: 'DATABASE', label: 'Chỉ Cơ sở dữ liệu (DATABASE)', count: counts.db },
                      { key: 'UPLOADS', label: 'Chỉ Tập tin tải lên (UPLOADS)', count: counts.uploads },
                      { key: 'SAFETY', label: 'Snapshot An toàn (SAFETY)', count: counts.safety },
                    ].map((item) => (
                      <button
                        key={item.key}
                        type="button"
                      ...
                      >
                        <div className="min-w-0 pr-2">
                          <div className={`text-xs font-semibold ${filterType === item.key ? 'text-blue-700 dark:text-blue-300 font-bold' : 'text-slate-800 dark:text-slate-200'}`}>
                            {item.label}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          {filterType === item.key && <Check className="h-4 w-4 text-blue-600 dark:text-blue-400" />}
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                {activeCategory === 'statuses' && (
                  <div className="space-y-1.5">
                    {[
                      ...
                      { key: 'SUCCEEDED', label: 'Thành công (SUCCEEDED)' },
                      { key: 'RUNNING', label: 'Đang chạy (RUNNING)' },
                      { key: 'VERIFYING', label: 'Đang kiểm tra (VERIFYING)' },
                      { key: 'QUEUED', label: 'Đang chờ (QUEUED)' },
                      { key: 'FAILED', label: 'Thất bại (FAILED)' },
                    ].map((item) => (
                      <button
                        key={item.key}
                        type="button"
                      ...
                      >
                        <div className="min-w-0 pr-2">
                          <div className={`text-xs font-semibold ${filterStatus === item.key ? 'text-blue-700 dark:text-blue-300 font-bold' : 'text-slate-800 dark:text-slate-200'}`}>
                            {item.label}
                          </div>
                        </div>
                ...
                <div className="text-[12px] font-medium text-slate-600 dark:text-slate-300">
                  {totalFilteredCount !== undefined ? (
                    <>
                      ...
                      {jobs.length > 0 && (
                        <span className="text-slate-400 dark:text-slate-500 font-normal"> / {jobs.length} snapshot ({Math.round((totalFilteredCount / Math.max(1, jobs.length)) * 100)}%)</span>
                      )}
                    </>
                  ) : (
                    'Đã áp dụng bộ lọc'
