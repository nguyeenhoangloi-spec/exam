...
interface ClassFilterPopoverProps {
  selectedDeptId: string;
  onDeptChange: (val: string) => void;
  selectedSizeRange?: string;
  onSizeRangeChange?: (val: string) => void;
...
type FilterCategory = 'presets' | 'department' | 'size';
...
export function ClassFilterPopover({
  ...
  const optionCounts = useMemo(() => {
    ...
    departments.forEach((d) => {
      deptCounts[String(d.id)] = classes.filter((c) => String(c.departmentId) === String(d.id)).length;
    });

    const getStudentCount = (c: any) => c._count?.students ?? c.studentsCount ?? c.students?.length ?? 0;

    const under20 = classes.filter((c) => getStudentCount(c) < 20).length;
    const from20to40 = classes.filter((c) => {
      const s = getStudentCount(c);
      return s >= 20 && s <= 40;
    }).length;
    const over40 = classes.filter((c) => getStudentCount(c) > 40).length;

    return { total, deptCounts, under20, from20to40, over40 };
  }, [classes, departments]);

  const activeFilterCount = [
    Boolean(selectedDeptId),
    Boolean(selectedSizeRange),
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      else if (e.key === '3') setActiveCategory('size');
    };
  ...
  const categories = [
    ...
    { id: 'department' as FilterCategory, label: 'Khoa / Viện', shortcut: '2', icon: Building2, badge: selectedDeptId ? '1' : null },
    { id: 'size' as FilterCategory, label: 'Sĩ số lớp', shortcut: '3', icon: Users, badge: selectedSizeRange ? '1' : null },
  ];

  const sizeList = [
    { value: '', label: 'Tất cả quy mô', desc: 'Mọi sĩ số lớp học', count: optionCounts.total },
    { value: 'over40', label: 'Lớp đông (> 40 SV)', desc: 'Lớp học phần quy mô lớn', count: optionCounts.over40 },
    { value: '20to40', label: 'Quy mô vừa (20 - 40 SV)', desc: 'Lớp học phần tiêu chuẩn', count: optionCounts.from20to40 },
    { value: 'under20', label: 'Quy mô nhỏ (< 20 SV)', desc: 'Lớp chuyên ban / Đồ án', count: optionCounts.under20 },
  ];

  const applyPreset = (presetType: string) => {
    if (presetType === 'ALL') {
      onResetAll();
    } else if (presetType === 'OVER40') {
      onSizeRangeChange?.('over40');
      onDeptChange('');
    } else if (presetType === '20TO40') {
      onSizeRangeChange?.('20to40');
      onDeptChange('');
  ...
  return (
    <div className="relative inline-block">
      ...
      <button
        ...
        title="Mở bảng điều khiển bộ lọc lớp học"
      >
      ...
      {isOpen && mounted &&
        createPortal(
          <div
            ...
            style={popoverStyle}
            role="dialog"
            aria-label="Bảng bộ lọc lớp học"
          ...
          >
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-type-helper text-slate-500 dark:text-slate-400 mt-0.5">
                    Tùy chỉnh tiêu chí phân loại lớp học
                  </p>
                </div>
              </div>
              ...
              <div className="col-span-8 p-3 overflow-y-auto custom-scrollbar">
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      <div className="min-w-0 pr-2">
                        <div className={`text-type-helper font-semibold ${selectedSizeRange === 'over40' ? 'text-blue-700 dark:text-blue-300' : 'text-slate-800 dark:text-slate-200'}`}>
                          Lớp đông sinh viên (&gt; 40 SV)
                        </div>
                        <div className={`text-type-helper truncate ${selectedSizeRange === 'over40' ? 'text-blue-600/80 dark:text-blue-300/80' : 'text-slate-500 dark:text-slate-400'}`}>
                          Lớp có quy mô sinh viên lớn cần phòng thi rộng
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        {selectedSizeRange === 'over40' && <Check className="h-4 w-4 text-blue-600 dark:text-blue-400" />}
                      </div>
                    </button>

                    <button
                      type="button"
                      onClick={() => applyPreset('20TO40')}
                      className={`flex w-full items-center justify-between rounded-xl border p-2.5 text-left transition-all duration-150 cursor-pointer ${
                    ...
                    >
                      <div className="min-w-0 pr-2">
                        <div className={`text-type-helper font-semibold ${selectedSizeRange === '20to40' ? 'text-blue-700 dark:text-blue-300' : 'text-slate-800 dark:text-slate-200'}`}>
                          Quy mô vừa (20 - 40 SV)
                        </div>
                        <div className={`text-type-helper truncate ${selectedSizeRange === '20to40' ? 'text-blue-600/80 dark:text-blue-300/80' : 'text-slate-500 dark:text-slate-400'}`}>
                          Quy mô lớp học phần phổ biến
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        {selectedSizeRange === '20to40' && <Check className="h-4 w-4 text-blue-600 dark:text-blue-400" />}
                      </div>
                    </button>
                    ...
                    {departments.length > 0 && (
                      ...
                      >
                        <div className="min-w-0 pr-2">
                          <div className={`text-type-helper font-semibold truncate ${selectedDeptId === String(departments[0].id) ? 'text-blue-700 dark:text-blue-300' : 'text-slate-800 dark:text-slate-200'}`}>
                            {departments[0].name}
                          </div>
                          <div className={`text-type-helper truncate ${selectedDeptId === String(departments[0].id) ? 'text-blue-600/80 dark:text-blue-300/80' : 'text-slate-500 dark:text-slate-400'}`}>
                            Các lớp thuộc {departments[0].name}
                          </div>
                        </div>
                ...
                {activeCategory === 'department' && (
                  <div className="space-y-1.5">
                    ...
                    >
                      <div className="min-w-0 pr-2">
                        <div className={`text-type-helper font-semibold ${selectedDeptId === '' ? 'text-blue-700 dark:text-blue-300 font-semibold' : 'text-slate-800 dark:text-slate-200'}`}>
                          Tất cả các khoa / viện
                        </div>
                        <div className={`text-type-helper truncate ${selectedDeptId === '' ? 'text-blue-600/80 dark:text-blue-400/80' : 'text-slate-400 dark:text-slate-500'}`}>
                          Toàn bộ lớp học trong hệ thống
                        </div>
                      </div>
                ...
                {activeCategory === 'size' && (
                  <div className="space-y-1.5">
                    {sizeList.map((item) => {
                      const isSelected = selectedSizeRange === item.value;
                      return (
                        <button
                          key={item.value}
                          type="button"
                          onClick={() => onSizeRangeChange?.(item.value)}
                          className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left transition-all duration-150 cursor-pointer border ${
                            isSelected
                              ? 'border-blue-500 bg-blue-50/80 dark:bg-blue-950/60 dark:border-blue-500 shadow-2xs'
                              : 'border-transparent hover:border-slate-200 dark:hover:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/60'
            ...
            <div className="shrink-0 flex items-center justify-between px-4 py-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/40">
              <div className="space-y-1">
                <div className="text-type-helper font-medium text-slate-600 dark:text-slate-300">
                  {totalFilteredCount !== undefined ? (
                    <>
                      ...
                      {classes.length > 0 && (
                        <span className="text-slate-400 dark:text-slate-500 font-normal"> / {classes.length} lớp học ({Math.round((totalFilteredCount / Math.max(1, classes.length)) * 100)}%)</span>
                      )}
                    </>
                  ) : (
                    'Đã áp dụng bộ lọc'
                ...
                {totalFilteredCount !== undefined && classes.length > 0 && (
                  <div className="h-1 w-28 rounded-full bg-slate-200/80 dark:bg-slate-700 overflow-hidden">
                    <div
                      className="h-full bg-blue-600 transition-all duration-300 rounded-full"
                      style={{ width: `${Math.min(100, Math.round((totalFilteredCount / Math.max(1, classes.length)) * 100))}%` }}
                    />
                  </div>
                )}
              </div>
