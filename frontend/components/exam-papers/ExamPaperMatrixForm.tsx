    ...
    <form onSubmit={onSubmit} className="rounded-2xl border border-slate-200/90 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-slate-100">
        ...
        <div className="relative inline-flex items-center rounded-full bg-slate-100 p-1 border border-slate-200 shrink-0">
          ...
          <button
            ...
            className={`relative z-10 rounded-full px-4 py-1.5 text-xs font-semibold transition-colors duration-200 cursor-pointer ${formData.durationMinutes !== '90' ? 'text-white' : 'text-slate-700 hover:text-slate-900'
              }`}
          >
            60 phút
          </button>
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 items-start">
          ...
          {/* 2. Mã đề gốc */}
          <div className="space-y-1.5">
            <label className="block text-[15px] font-medium text-slate-900">Mã đề gốc</label>
            <input
              type="text"
              required
          ...
          {/* 3. Số mã đảo */}
          <div className="space-y-1.5">
            <label className="block text-[15px] font-medium text-slate-900">Số mã đảo</label>
            <input
              type="number"
              min={1}
              max={10}
          ...
          {/* 4. Thời gian */}
          <div className="space-y-1.5">
            <label className="block text-[15px] font-medium text-slate-900">Thời gian (phút)</label>
            <input
              type="number"
              min={15}
        ...
        <div className="space-y-1.5">
          <label className="block text-[15px] font-medium text-slate-500 tracking-wider">
                        ...
                        pending.map((s: any) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className="text-[12px] text-slate-500 dark:text-slate-400 font-semibold mt-0.5 truncate">
                                {periodName} · {fmt(s.examDate)}
                              </p>
                              <p className="text-[12px] text-slate-400 dark:text-slate-500 mt-0.5">{s.startTime} – {s.endTime}</p>
                            </button>
                          );
                        })
                      )}
                            ...
                            >
                              <div className="flex items-center gap-1.5">
                                ...
                                {s.examPapers?.some((p: any) => p.status === 'PUBLISHED') && (
                                  <span className="inline-flex items-center gap-[6px] shrink-0 text-[13px] leading-5 font-semibold text-success-600">
        ...
        <div className="space-y-3 pt-1 border-t border-slate-100">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-4 text-xs font-semibold border-b border-slate-200 shrink-0">
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                ...
                <div key={key} className="space-y-1">
                  <label className="block text-[15px] font-medium text-slate-700">{lb}</label>
                  <input
                    type="number"
                    step="0.25"
                    min={0}
