    ...
    <div className={`rounded-2xl border bg-white shadow-2xs overflow-hidden ${borderCls}`}>
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className={`text-xs font-black ${showAnswer && q.type !== 'ESSAY' ? (isCorrect ? 'text-[#15803D]' : selectedIds.length === 0 ? 'text-[#475569]' : 'text-[#DC2626]') : 'text-[#2563EB]'}`}>
                ...
                {((q.fillBlankAnswers && q.fillBlankAnswers.length > 0 ? q.fillBlankAnswers : [{ blankIndex: 1 }]) as any[]).map((expected: any) => {
                  ...
                  return (
                    <div key={bIdx} className="flex flex-wrap items-center justify-between gap-2 bg-white border border-slate-200 p-2.5 rounded-lg text-xs">
                      ...
                      {showAnswer && (
                        <div className="inline-flex items-center gap-[6px] text-xs font-semibold text-[#15803D]">
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
      <div className="relative w-full max-w-3xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        <div className="flex items-start justify-between gap-4 px-6 py-4 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shrink-0">
          <div>
            <h2 className="text-lg font-semibold leading-snug text-slate-900 dark:text-slate-100">Xem Lại Chi Tiết Bài Thi</h2>
            {data && (
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 hover:bg-slate-200/70 dark:hover:bg-slate-700 hover:text-slate-700 dark:hover:text-slate-200 transition cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
