...
export function ExamReportSummaryTab({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Sub-navigation Toolbar (Phẳng, không lồng khung) ── */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <SlidingSegmentedControl<'overview' | 'builder' | 'history'>
          ...
          options={[
            { value: 'overview', label: 'Mẫu báo cáo thường dùng', icon: BarChart3 },
            { value: 'builder', label: 'Tạo báo cáo tùy biến', icon: Settings2 },
        ...
        {tab === 'overview' && (
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="h-10 w-full rounded-xl border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 pl-10 pr-3 text-type-body font-normal text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-blue-500 transition shadow-2xs"
            />
          </div>
        )}
      </div>

      {/* ── TAB 1: MẪU BÁO CÁO NHANH & BẢNG CA THI GẦN ĐÂY ── */}
      {tab === 'overview' && (
        <div className="space-y-4">
          {/* Dải nút chọn mẫu báo cáo nhanh */}
          {catalog.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 pt-1">
              <span className="text-type-helper font-medium text-slate-500 dark:text-slate-400 mr-1">
                Mẫu nhanh:
              </span>
              {catalog.map((item) => (
                <button
                  ...
                  type="button"
                  onClick={() => choose(item)}
                  className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 text-type-body-sm font-medium text-slate-700 dark:text-slate-300 hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 transition-all shadow-2xs cursor-pointer"
                >
                  <span>{item.name}</span>
