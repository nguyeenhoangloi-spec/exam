              ...
              <div className="col-span-8 p-3 overflow-y-auto custom-scrollbar">
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      <div className="min-w-0 pr-2">
                        <div className={`text-xs font-semibold ${filters.status === 'ONGOING' ? 'text-blue-700 dark:text-blue-300' : 'text-slate-800 dark:text-slate-200'}`}>
                          Ca thi đang diễn ra
                        </div>
                        <div className={`text-[12px] truncate ${filters.status === 'ONGOING' ? 'text-blue-600/80 dark:text-blue-300/80' : 'text-slate-500 dark:text-slate-400'}`}>
                          Các phòng thi đang mở hôm nay
                        </div>
                      </div>

                      <div className="flex items-center gap-2 shrink-0">
                        <span className={`rounded-md px-1.5 py-0.5 text-[12px] font-semibold ${filters.status === 'ONGOING'
                    ...
                    >
                      <div className="min-w-0 pr-2">
                        <div className={`text-xs font-semibold ${filters.status === 'UPCOMING' ? 'text-blue-700 dark:text-blue-300' : 'text-slate-800 dark:text-slate-200'}`}>
                          Lịch thi sắp tới
                        </div>
                        <div className={`text-[12px] truncate ${filters.status === 'UPCOMING' ? 'text-blue-600/80 dark:text-blue-300/80' : 'text-slate-500 dark:text-slate-400'}`}>
                          Chuẩn bị tổ chức theo kế hoạch
                        </div>
                      </div>

                      <div className="flex items-center gap-2 shrink-0">
                        <span className={`rounded-md px-1.5 py-0.5 text-[12px] font-semibold ${filters.status === 'UPCOMING'
                            ? 'bg-blue-600 text-white'
                            : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'
                          }`}>
                    ...
                    {periods.length > 0 && (
                      ...
                      >
                        <div className="min-w-0 pr-2">
                          <div className={`text-xs font-semibold truncate ${filters.examPeriodId === String(periods[0].id) ? 'text-blue-700 dark:text-blue-300' : 'text-slate-800 dark:text-slate-200'}`}>
                            {periods[0].name}
                          </div>
                          <div className={`text-[12px] truncate ${filters.examPeriodId === String(periods[0].id) ? 'text-blue-600/80 dark:text-blue-300/80' : 'text-slate-500 dark:text-slate-400'}`}>
                            Kỳ thi hiện hành mới nhất
                          </div>
                        </div>

                        <div className="flex items-center gap-2 shrink-0">
                          <span className={`rounded-md px-1.5 py-0.5 text-[12px] font-semibold ${filters.examPeriodId === String(periods[0].id)
                              ? 'bg-blue-600 text-white'
                              : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'
                            }`}>
                ...
                {activeCategory === 'period' && (
                  <div className="space-y-1.5">
                    ...
                    >
                      <div className="min-w-0 pr-2">
                        <div className={`text-xs font-semibold ${filters.examPeriodId === '' ? 'text-blue-700 dark:text-blue-300 font-semibold' : 'text-slate-800 dark:text-slate-200'}`}>
                          Tất cả kỳ thi
                        </div>
                        <div className={`text-[12px] truncate ${filters.examPeriodId === '' ? 'text-blue-600/80 dark:text-blue-400/80' : 'text-slate-400 dark:text-slate-500'}`}>
                          Mọi đợt thi trong hệ thống
                        </div>
                      </div>

                      <div className="flex items-center gap-2 shrink-0">
                        <span className={`rounded-md px-1.5 py-0.5 text-[12px] font-semibold ${filters.examPeriodId === ''
                            ? 'bg-blue-600 text-white'
                    ...
                    {periods.map((p) => {
                      ...
                      return (
                        ...
                        >
                          <div className="min-w-0 pr-2">
                            <div className={`text-xs font-semibold truncate ${isSelected ? 'text-blue-700 dark:text-blue-300 font-semibold' : 'text-slate-800 dark:text-slate-200'}`}>
                              {p.name}
                            </div>
                ...
                {activeCategory === 'room' && (
                  <div className="space-y-1.5">
                    ...
                    >
                      <div className="min-w-0 pr-2">
                        <div className={`text-xs font-semibold ${filters.roomId === '' ? 'text-blue-700 dark:text-blue-300 font-semibold' : 'text-slate-800 dark:text-slate-200'}`}>
                          Tất cả phòng thi
                        </div>
                        <div className={`text-[12px] truncate ${filters.roomId === '' ? 'text-blue-600/80 dark:text-blue-400/80' : 'text-slate-400 dark:text-slate-500'}`}>
                          Mọi phòng thi được cấu hình
                        </div>
                      </div>

                      <div className="flex items-center gap-2 shrink-0">
                        <span className={`rounded-md px-1.5 py-0.5 text-[12px] font-semibold ${filters.roomId === ''
                            ? 'bg-blue-600 text-white'
                            : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400'
                          }`}>
                          {optionCounts.total}
