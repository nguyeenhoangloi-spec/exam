...
import { Clock, Send, X, Users } from 'lucide-react';
      ...
      <div className="pointer-events-auto max-w-full bg-white/95 dark:bg-slate-900/95 border border-slate-200/60 dark:border-slate-800 shadow-[0_20px_50px_-12px_rgba(15,23,42,0.2)] dark:shadow-[0_24px_60px_-12px_rgba(0,0,0,0.6)] rounded-2xl p-2 px-4 sm:px-5 backdrop-blur-xl ring-1 ring-slate-900/5 dark:ring-white/10 flex flex-wrap items-center justify-center gap-3 sm:gap-4 animate-in slide-in-from-bottom-5 duration-300">
        ...
        <div className="flex items-center gap-2.5 pr-3.5 border-r border-slate-200 dark:border-slate-800 shrink-0">
          <div className="inline-flex items-center gap-2.5 rounded-xl bg-blue-50 dark:bg-blue-950/60 border border-blue-200/80 dark:border-blue-800/60 px-3 py-1.5 text-type-helper text-blue-600 dark:text-blue-400 select-none shadow-2xs">
            ...
            {/* Thanh tiến độ mini cho biết tỷ lệ thí sinh đã chọn */}
            <div className="w-9 h-1.5 bg-blue-200/70 dark:bg-blue-900/70 rounded-full overflow-hidden hidden sm:block" title={`Đã chọn ${progressPercent}% thí sinh`}>
              <div
        ...
        <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2">
          {/* Gia hạn thời gian: Primary Blue chủ đạo */}
          <button
            type="button"
            onClick={onExtend}
          ...
          {/* Gửi thông báo tới nhóm đã chọn: Nút Phẳng (Flat Ghost) */}
          {onBroadcast && (
            <button
              type="button"
              onClick={onBroadcast}
