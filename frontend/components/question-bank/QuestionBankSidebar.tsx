    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-2xs space-y-3">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
          <h3 className="text-[18px] font-semibold text-[#0F172A]">Tổng quan ngân hàng</h3>
          <div className="relative">
            <select className="appearance-none rounded-xl border border-slate-200 bg-white px-3 py-1.5 pr-7 text-[13px] font-semibold text-[#334155] outline-none hover:bg-slate-50 cursor-pointer shadow-2xs">
              <option>Tất cả thời gian</option>
              <option>Tháng này</option>
              <option>Học kỳ này</option>
            </select>
            <ChevronDown className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-[#64748B]" />
          </div>
        </div>
            ...
            {statusData.map((item) => (
              <div key={item.name} className="flex items-center justify-between">
                ...
                <span className="font-semibold text-[#0F172A]">
                  ...
                  <span className="text-[13px] text-[#64748B] font-normal">({item.percent})</span>
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-2xs space-y-3">
        <h3 className="text-[18px] font-semibold text-[#0F172A] border-b border-slate-100 pb-2.5">
          Phân bố độ khó
        </h3>
      ...
      <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-2xs space-y-2.5">
        ...
        <div className="space-y-1.5 pt-1">
          ...
          <button
            type="button"
            onClick={onImport}
            className="flex w-full items-center justify-between rounded-xl px-3 py-2 text-[15px] font-medium text-[#334155] transition hover:bg-slate-50 cursor-pointer"
          >
            <span className="flex items-center gap-2">
