      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`group relative flex items-center gap-2 px-4 py-3 text-[15px] font-medium transition-all cursor-pointer whitespace-nowrap ${
              isActive ? 'text-[#2563EB] font-semibold' : 'text-[#64748B] dark:text-slate-400 hover:text-[#0F172A] dark:hover:text-slate-100'
            }`}
          >
            <span>{tab.label}</span>
            <span
              className={`rounded-full px-2.5 py-0.5 text-[13px] font-semibold transition ${
                isActive
                  ? 'bg-blue-50 dark:bg-blue-950/60 text-[#2563EB]'
                  : 'bg-slate-100 dark:bg-slate-800 text-[#64748B] dark:text-slate-400 group-hover:bg-slate-200 dark:group-hover:bg-slate-700'
              }`}
            >
