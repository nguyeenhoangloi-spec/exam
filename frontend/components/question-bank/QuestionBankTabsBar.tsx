      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`group relative flex items-center gap-2 px-4 py-3 text-type-body font-medium transition-all cursor-pointer whitespace-nowrap ${isActive ? 'text-primary-600 font-semibold' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100'
              }`}
          >
            <span>{tab.label}</span>
            <span
              className={`ui-pill rounded-full border px-2.5 py-0.5 text-type-helper font-medium transition ${isActive
