...
export function QuestionCard({
 ...
 {q.chapter?.name && (
 <span className="rounded-lg bg-slate-100 border border-slate-200 px-2.5 py-0.5 text-[13px] font-medium text-slate-700 h-6 inline-flex items-center">
 {q.chapter.name}
 </span>
 )}
 ...
 <span className="text-[14px] font-semibold text-slate-900 ml-1">{q.score || (q.type === 'ESSAY' ? 1.0 : 0.25)}đ</span>
 </div>
 ...
 <span className="text-[13px] font-medium text-slate-500 max-w-[140px] truncate">
 ...
 <div className="mt-1 flex items-center justify-center gap-1 text-[13px] font-normal text-slate-500">
 ...
 <span className="text-[13px] font-semibold tracking-wider text-slate-500">
 Lựa chọn ({options.length} phương án)
 </span>
 <button
 onClick={() => setShowOptions(!showOptions)}
 className="text-[13px] font-medium text-primary-600 hover:text-blue-700 flex items-center gap-1 transition"
 >
 {showOptions ? (
 ...
 <div
 key={opt.id || idx}
 className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50/80 p-2.5 text-[15px] font-normal text-slate-700 transition"
 >
 <div className="flex items-center gap-2.5 min-w-0 pr-2">
 <span
 className="flex h-6 min-w-6 shrink-0 items-center justify-center rounded-lg bg-slate-200 px-1.5 text-[13px] font-semibold text-slate-900"
 >
 {label}
 </span>
 ...
 <div className="rounded-xl bg-blue-50/70 border border-blue-200/80 p-3 text-[14px] text-slate-700 flex items-start gap-2">
 <HelpCircle className="w-4 h-4 text-primary-600 shrink-0 mt-0.5" />
 <div>
 <strong className="font-semibold text-slate-900">Giải thích: </strong>
 ...
 {/* Footer Info */}
 <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[14px] text-slate-500 font-normal">
 <span>
 Người tạo: <strong className="text-slate-900 font-medium">{q.createdBy?.username || '—'}</strong>
 </span>
 <span>
