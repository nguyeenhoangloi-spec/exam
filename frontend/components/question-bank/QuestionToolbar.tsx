...
export function QuestionToolbar({ onAdd, onImport, onAi, onExport, onPrint }: { onAdd: () => void; onImport: () => void; onAi: () => void; onExport: () => void; onPrint?: () => void }) {
  return (
    <div className="flex flex-wrap gap-2">
      {onPrint && (
        <button onClick={onPrint} className="flex items-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-700 font-bold px-3.5 py-2 text-sm text-white shadow-xs transition">
      ...
      <button onClick={onAi} className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-sky-600 hover:from-blue-700 hover:to-sky-700 px-3.5 py-2 text-sm font-bold text-white transition shadow-xs">
        <Sparkles className="h-4 w-4" />
        Tạo bằng AI
      </button>
      <button onClick={onAdd} className="flex items-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-700 px-3.5 py-2 text-sm font-bold text-white transition shadow-xs">
