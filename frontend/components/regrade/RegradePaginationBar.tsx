...
export function RegradePaginationBar({
  page,
  totalPages,
  limit,
  totalItems,
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between pt-3">
      <p className="text-[14px] font-normal text-slate-500">
        ...
        <span className="font-semibold text-slate-900">{totalItems.toLocaleString('vi-VN')}</span> đơn phúc khảo
      </p>
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1">
          ...
          {pages.map((p, idx) => {
            if (p === '...') {
              return (
                <span key={`dots-${idx}`} className="px-1 text-[14px] font-medium text-slate-500">
                  ...
                </span>
              );
            }
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none rounded-xl border border-slate-200 bg-white pl-3 pr-7 py-1.5 text-[14px] font-medium text-slate-900 outline-none hover:bg-slate-50 transition cursor-pointer shadow-2xs"
          >
