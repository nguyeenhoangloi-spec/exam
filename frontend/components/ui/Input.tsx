  ...
  ({ label, error, helperText, leftIcon, rightIcon, className = '', containerClassName = '', id, type = 'text', ...props }, ref) => {
    ...
    return (
      <div className={`space-y-1.5 w-full ${containerClassName}`}>
        {label && (
          <label htmlFor={inputId} className="block text-[15px] font-medium text-slate-700 dark:text-slate-200">
            {label}
        ...
        {error ? (
          <p className="text-[13px] font-medium text-danger-600 pl-0.5">{error}</p>
        ) : helperText ? (
          <p className="text-[13px] font-normal text-slate-500 dark:text-slate-400 pl-0.5">{helperText}</p>
        ) : null}
      </div>
    );
  },
    ...
    <Input
      ...
      rightIcon={
        <button
          ...
          className="text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 cursor-pointer p-1"
        >
...
export const Select = React.forwardRef<HTMLSelectElement, SelectProps>(
  ({ label, error, helperText, containerClassName = '', className = '', children, options, id, ...props }, ref) => {
    ...
    return (
      <div className={`space-y-1.5 w-full ${containerClassName}`}>
        {label && (
          <label htmlFor={selectId} className="block text-[15px] font-medium text-slate-700 dark:text-slate-200">
            {label}
            {props.required && <span className="text-danger-600 ml-1">*</span>}
          </label>
        )}
...
export const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ label, error, helperText, containerClassName = '', className = '', id, ...props }, ref) => {
    ...
    return (
      <div className={`space-y-1.5 w-full ${containerClassName}`}>
        {label && (
          <label htmlFor={textareaId} className="block text-[15px] font-medium text-slate-700 dark:text-slate-200">
            {label}
            {props.required && <span className="text-danger-600 ml-1">*</span>}
          </label>
        )}
